...
<template>
  ...
  <el-dialog v-model="outerVisible" title="菜单权限" class="el-dialog" :show-close="false">
    <span>路由信息</span>
    <el-form :model="form" label-width="150px"   :rules="rules"  ref="ruleFormRef">
...
name:[{validator: (rule: any, value: any, callback: any)=>{
    if (value === '') {
...
parentPath:[{validator: (rule: any, value: any, callback: any)=>{
    if (value === '') {
...
watch(iconsDialog,(newValue,oldValue)=>{
  ...
  icontext.then((icontext)=>{
    ...
    form.meta.icon = icontext.replace("<el-icon><",'').replace("/></el-icon>",'').trim().toLowerCase()
