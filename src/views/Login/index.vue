    ...
    <div class="loginMain">
      <el-form :model="loginUser" ref="loginForm" :rules="loginRules" class="loginForm">
        ...
        <el-form-item :rules="[{ required: true, message: '请输入验证码', trigger: 'blur' }]">
          ...
          <el-image  :src="imgBase" fit="fill" @click="getCodeImg" alt="加载失败" />
          </div>
        </el-form-item>
        <el-form-item>
...
import { Calendar, Search, User, View, Lock, MagicStick } from '@element-plus/icons-vue'
...
const imgBase = ref('');// 验证码base64
const getCodeImg = ()=>{
  getCode().then((data)=>{
    const {msg,uuid,img} = data
    loginUser.uuid = uuid;
    imgBase.value = "data:image/jpeg;base64,"+ img;
  })
}
onMounted(() => {
  getCodeImg();
...
const login = (loginForm: FormInstance | undefined) => {
  ...
  loginForm.validate(valid => {
    try {
      loginApi({ ...loginUser }).then(async (data) => {
        const gobalStote = GlobalStore();
        gobalStote.setToken(data?.token)
